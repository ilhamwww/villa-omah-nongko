...
@php
    $title = $title ?? __('Siap merasakan pengalaman di Omah Nongko?');
    $subtitle = $subtitle ?? __('Pesan tempat menginap Anda dan nikmati liburan privat di Yogyakarta.');
...
<section class="bg-primary text-white relative overflow-hidden">
    <div class="container-site py-12 md:py-16">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
                <h2 class="font-heading text-3xl md:text-4xl leading-tight text-white">{{ $title }}</h2>
                <p class="mt-3 text-sm md:text-base text-white/70 max-w-lg">{{ $subtitle }}</p>
            </div>
            <a href="{{ $waLink }}" target="_blank" rel="noopener noreferrer" class="btn-outline-light shrink-0">
