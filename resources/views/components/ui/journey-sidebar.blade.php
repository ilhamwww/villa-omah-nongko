...
<aside class="space-y-8" aria-label="Sidebar cerita">
    ...
    <form action="{{ route('journey.index') }}" method="GET" role="search">
        <label for="journey-search" class="sr-only">{{ __('Cari artikel') }}</label>
        <div class="relative">
            <input id="journey-search" type="search" name="q" value="{{ $search }}" placeholder="{{ __('Cari artikel...') }}"
                   class="w-full h-14 border border-border-light bg-bg-card pl-4 pr-12 text-sm focus:border-text-muted focus:ring-0">
            <button type="submit" aria-label="{{ __('Cari') }}" class="absolute right-4 top-1/2 -translate-y-1/2 text-text-muted">
    ...
    <div class="border border-border-light bg-bg-card p-6">
        <h2 class="font-heading text-xl">{{ __('Kategori') }}</h2>
    ...
    {{-- Artikel Populer --}}
    <div class="border border-border-light bg-bg-card p-6">
        <h2 class="font-heading text-xl">{{ __('Artikel Populer') }}</h2>
    ...
    {{-- Berlangganan --}}
    <div class="border border-border-light bg-bg-card p-6">
        <h2 class="font-heading text-xl">{{ __('Tetap Terinspirasi') }}</h2>
        <p class="mt-2 text-sm text-text-muted">{{ __('Masukkan email Anda untuk menerima tips wisata, penawaran spesial, dan info terbaru villa.') }}</p>
        <form x-data="{ email: '', terkirim: false }" x-on:submit.prevent="terkirim = true" class="mt-4 space-y-3">
            <label for="sidebar-email" class="sr-only">{{ __('email_address') }}</label>
            <input x-model="email" id="sidebar-email" type="email" required placeholder="{{ __('enter_your_email') }}"
                   class="w-full h-12 border border-border-light px-3 text-sm focus:border-text-muted focus:ring-0">
            <button type="submit" class="btn-primary w-full" x-text="terkirim ? '{{ __('subscribed') }}' : '{{ __('subscribe') }}'">{{ __('subscribe') }}</button>
        </form>
    </div>
    ...
    <div class="relative overflow-hidden bg-primary text-white p-8">
        ...
        <h2 class="font-heading text-2xl text-white relative">{{ __('Siap merasakan Omah Nongko?') }}</h2>
