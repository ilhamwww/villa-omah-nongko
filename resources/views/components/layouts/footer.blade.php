...
@php
    ...
    $jelajahi = [
        ['label' => __('Tentang Villa'), 'url' => route('the-villa')],
        ['label' => __('Kamar & Suite'), 'url' => route('the-villa') . '#suites'],
        ['label' => __('Fasilitas'), 'url' => route('home.index') . '#amenities'],
        ['label' => __('Pengalaman'), 'url' => route('home.index') . '#experiences'],
        ['label' => __('Galeri'), 'url' => route('gallery')],
        ['label' => __('Cerita'), 'url' => route('journey.index')],
    ];
    $informasi = [
        ['label' => __('Harga & Ketersediaan'), 'url' => $waLink],
        ['label' => __('Lokasi'), 'url' => $identity['google_maps_url']],
        ['label' => __('Tata Tertib'), 'url' => '#'],
        ['label' => __('Pertanyaan Umum'), 'url' => '#'],
        ['label' => __('Hubungi Kami'), 'url' => 'mailto:' . $identity['email']],
    ];
@endphp
...
<footer class="{{ $isDark ? 'bg-primary text-white' : 'bg-bg-soft text-text-main border-t border-border-light' }}">
    <div class="container-site py-section-md">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1.2fr] gap-10 lg:gap-16">
            ...
            {{-- Jelajahi --}}
            <div>
                <h3 class="eyebrow {{ $isDark ? 'text-white/60' : '' }}">{{ __('Jelajahi') }}</h3>
            ...
            {{-- Hubungi Kami --}}
            <div>
                <h3 class="eyebrow {{ $isDark ? 'text-white/60' : '' }}">{{ __('Hubungi Kami') }}</h3>
        ...
        <div class="mt-12 pt-6 border-t {{ $isDark ? 'border-white/15' : 'border-border-light' }} flex flex-col sm:flex-row items-center justify-between gap-4 text-xs {{ $isDark ? 'text-white/50' : 'text-text-soft' }}">
            <p>&copy; {{ date('Y') }} {{ $identity['site_name'] }}. {{ __('Hak cipta dilindungi.') }}</p>
            <div class="flex items-center gap-6">
                <a href="#" class="hover:underline">{{ __('Kebijakan Privasi') }}</a>
                <a href="#" class="hover:underline">{{ __('Syarat & Ketentuan') }}</a>
            </div>
        </div>
    </div>
</footer>