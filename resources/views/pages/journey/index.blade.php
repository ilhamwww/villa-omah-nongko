@php
    ...
    $judulHero = $halaman?->hero_title ?: __('Cerita Perjalanan');
    $deskripsiHero = $halaman?->hero_description ?: __('Cerita dan inspirasi dari Yogyakarta. Temukan pengalaman lokal, tips wisata, kuliner, dan kehidupan villa di Omah Nongko.');
    $fotoHero = $halaman?->hero_image ? asset('storage/'.$halaman?->hero_image) : $gambar['hero_journey'];
    $altFotoHero = $halaman?->hero_image_alt ?: __('Suasana Villa Omah Nongko untuk artikel dan cerita perjalanan Yogyakarta');
    $judulSEO = $halamanSaatIni > 1 ? __('Cerita Perjalanan') . " — " . __('page') . " {$halamanSaatIni}" : ($halaman?->seo_title ?: __('Cerita Perjalanan — Wisata, Budaya & Villa di Yogyakarta'));
    $schema = [
...
<x-layouts.app
    :title="$judulSEO"
    :description="$halaman?->seo_description ?: __('Cerita dan inspirasi dari Yogyakarta. Temukan pengalaman lokal, tips wisata, kuliner, dan kehidupan villa di Villa Omah Nongko.')"
    ...
    {{-- Daftar Artikel --}}
    <section class="py-section-md" aria-label="{{ __('latest_articles') }}">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-12 lg:gap-16 items-start">
                ...
                <div>
                    <div class="flex items-center justify-between mb-8 reveal-slide-up">
                        <h2 class="font-heading text-3xl md:text-4xl">{{ __('latest_articles') }}</h2>
                    </div>

                    @if($daftarArtikel->isEmpty())
                        <p class="text-text-muted reveal-fade">{{ __('no_articles_yet') }}</p>
                    @else
                        <div class="space-y-3">
                        ...
                        @if($daftarArtikel->hasPages())
                            <nav class="mt-10 flex items-center gap-5 reveal-fade" aria-label="{{ __('page_navigation') }}">
    ...
    {{-- Strip Fitur (gelap) --}}
    <section class="bg-primary text-white relative overflow-hidden" aria-label="{{ __('villa_amenities') }}">
            ...
            <div class="grid grid-cols-3 md:grid-cols-6 gap-y-8 gap-x-4">
                @foreach($stripFitur as $fitur)
                    <div class="flex flex-col items-center text-center reveal-slide-up" style="transition-delay: {{ ($loop->index % 6) * 100 }}ms;">
                        ...
                        <span class="mt-3 text-xs leading-snug text-white/80">{{ __($fitur['label']) }}</span>
                    </div>
                @endforeach
            </div>
        </div>
